<template>
  <div>
    <a-modal v-model="visible" :confirmLoading="loading" :maskClosable="false" @cancel="cancel" @ok="confirm">
      <div slot="title">{{ form.id ? "编辑备品备件" : "新增备品备件" }}</div>
      <div>
        <a-form-model ref="form" :model="form" :rules="rules" :label-col="{ span: 4 }" :wrapper-col="{ span: 16 }">
          ...
          <a-form-model-item prop="lower_quantity" label="最低库存">
            <a-input v-model="form.lower_quantity" allowClear />
          </a-form-model-item>
          <a-form-model-item prop="unit" label="单位">
            <a-input v-model="form.unit" allowClear />
          </a-form-model-item>
          <a-form-model-item prop="unit_price" label="单价">
            <a-input v-model="form.unit_price" allowClear />
          </a-form-model-item>
        </a-form-model>
      </div>
    </a-modal>
