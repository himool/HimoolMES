<template>
  <div>
    <a-modal v-model="visible" :confirmLoading="loading" :maskClosable="false" @cancel="cancel" @ok="confirm">
      <div slot="title">{{ form.id ? "编辑维修任务" : "新增维修任务" }}</div>
      <div>
        <a-form-model ref="form" :model="form" :rules="rules" :label-col="{ span: 4 }" :wrapper-col="{ span: 16 }">
          ...
          <a-form-model-item prop="time" label="发生时间">
            <a-input v-model="form.time" allowClear />
          </a-form-model-item>
          <a-form-model-item prop="remark" label="故障现象">
