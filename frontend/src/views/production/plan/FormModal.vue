    ...
    <a-modal v-model="visible" :confirmLoading="loading" :maskClosable="false" @cancel="cancel" @ok="confirm">
      ...
      <div>
        <a-form-model ref="form" :model="dataForm" :rules="rules" :label-col="{ span: 6 }" :wrapper-col="{ span: 16 }">
          <a-form-model-item prop="number" label="工单单号">
            <a-input v-model="dataForm.number" />
          </a-form-model-item>
          <a-form-model-item prop="material" label="物料">
            <a-select v-model="dataForm.material" style="width: 100%;">
              <a-select-option v-for="item in $parent.materialItems" :key="item.id" :value="item.id">
                {{ item.name }}
              </a-select-option>
            </a-select>
          </a-form-model-item>
          ...
          <!-- <a-form-model-item v-if="!dataForm.is_related" prop="goods" label="物料">
  ...
  data() {
    return {
      rules: {
        ...
        material: [{ required: true, message: "请选择生产物料", trigger: "change" }],
