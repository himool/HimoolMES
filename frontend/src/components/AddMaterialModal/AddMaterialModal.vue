<template>
  <div>
    <a-modal v-model="visible" title="选择物料" :footer="null" :maskClosable="false" @cancel="cancel">
      <div>
    ...
    <a-modal v-model="formVisible" :maskClosable="false" okText="确定" cancelText="取消" :title="targetItem.name"
      ...
      <a-form-model ref="form" :model="targetItem" :rules="rules" :label-col="{ span: 4 }" :wrapper-col="{ span: 16 }">
        <a-form-model-item prop="quantity" label="物料数量">
  ...
  export default {
    ...
    methods: {
      ...
      validateQuantity(rule, value, callback) {
        return value > 0 ? callback() : callback(new Error('物料数量需要大于0'))
      },
    },
    watch: {
      visible(value) {
